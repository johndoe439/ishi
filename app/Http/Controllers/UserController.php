...
class UserController extends Controller
{
    ...
    public function update(UpdateQuoteRequest $request, Quote $quote)
    {
        ...
        for ($i = 1; $i <= 4; $i++) {
            ...
            if ($request->has($removeKey) && $request->input($removeKey) == '1') {
                if ($currentImages["image_{$i}"]) {
                    \Storage::disk('public')->delete($currentImages["image_{$i}"]);
                    $currentImages["image_{$i}"] = null;
                }
            }
        }
        ...
        for ($i = 1; $i <= 4; $i++) {
            ...
            if ($request->hasFile($fileKey)) {
                ...
                // Store new image
                $path = $request->file($fileKey)->store('quote_images', 'public');
                $currentImages["image_{$i}"] = $path;
            }
        }

        // IMPORTANT: Remove any remove_image_* keys from validated data
        // (in case they slipped in due to custom rules or manual validation)
        foreach ($validated as $key => $value) {
            if (str_starts_with($key, 'remove_image_')) {
                unset($validated[$key]);
            }
        }

        // Merge only the real image columns
        $updateData = array_merge($validated, [
            'image_1' => $currentImages['image_1'],
            'image_2' => $currentImages['image_2'],
            'image_3' => $currentImages['image_3'],
            'image_4' => $currentImages['image_4'],
        ]);
